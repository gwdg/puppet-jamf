Puppet::Type.newtype(:jamf_advanced_computer_search) do
  desc 'Manages Advanced Computer Searches within a Jamf Pro Server'

  ensurable

  # namevar is always a parameter
  newparam(:name, namevar: true) do
    desc 'Name of the Advanced Computer Search'
  ...
  newparam(:api_url) do
    desc 'URL of the JAMF API. Note: we will append "/JSSResource/advancedcomputersearches" to the end of this. Default: https://127.0.0.1:8443'
  ...
  newparam(:search_name) do
    desc 'Name of the advanced computer search for cloud servers.'
    ...
    validate do |value|
      unless value.is_a?(String)
        raise ArgumentError, "search_name is expected to be a String, given: #{value.class.name}"
      end
    end
  end
