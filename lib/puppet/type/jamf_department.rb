Puppet::Type.newtype(:jamf_department) do
  desc 'Manages the departments available within a Jamf Pro Server'
  ...
  newparam(:api_url) do
    desc "URL of the JAMF API. Note: we will append '/JSSResource/departments' to the end of this. Default: https://127.0.0.1:8443"
  ...
  newparam(:department_name) do
    desc 'Name of the department for cloud servers.'
    ...
    validate do |value|
      unless value.is_a?(String)
        raise ArgumentError, "department_name is expected to be a String, given: #{value.class.name}"
      end
    end
  end
