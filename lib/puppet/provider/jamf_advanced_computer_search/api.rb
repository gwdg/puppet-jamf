...
Puppet::Type.type(:jamf_advanced_computer_search).provide(:api, parent: Puppet::Provider::Jamf) do
  ...
  def read_instance
    resp = authorized_http_client.get(adv_computer_search_url,
                                      headers: { 'Accept' => 'application/json' })
    body_json = JSON.parse(resp.body)
    searches_list = body_json['advanced_computer_searches']

    # find the category that matches our name
    matches = searches_list.select { |ls| ls['name'] == search_name }
    if matches.size >= 1
      search_id = matches.first['id']
      resp = authorized_http_client.get(adv_computer_search_url + "/id/#{search_id}",
                                        headers: { 'Accept' => 'application/json' })
      search = JSON.parse(resp.body)['advanced_computer_search']
      instance = {
        ensure: :present,
  ...
  def flush_instance
    ...
    when :absent
      ...
      unless cached_instance[:id].nil?
        url = adv_computer_search_url + "/id/#{cached_instance[:id]}"
    ...
    when :present
      ...
      if cached_instance[:id].nil?
        ...
        authorized_http_client.post(adv_computer_search_url + '/id/0',
                                    body: body,
                                    headers: { 'Content-Type' => 'application/xml' })
      else
        authorized_http_client.put(adv_computer_search_url + "/id/#{cached_instance[:id]}",
                                   body: body,
                                   headers: { 'Content-Type' => 'application/xml' })
      end
    end
  ...
  def adv_computer_search_url
    # create a URL based on our API URL
    @adv_computer_search_url ||= "#{resource[:api_url]}/JSSResource/advancedcomputersearches"
  end
  ...
  #       with a value of true along with an search_name attribute for cloud
  #       server management to operate correctly.
  def search_name
    resource[:is_cloud] ? resource[:search_name] : resource[:name]
  end
end
