...
Puppet::Type.type(:jamf_category).provide(:api, parent: Puppet::Provider::Jamf) do
  ...
  def read_instance
    resp = authorized_http_client.get(categories_url,
                                      headers: { 'Accept' => 'application/json' })
    body_json = JSON.parse(resp.body)
    categories_list = body_json['categories']

    # find the category that matches our name
    matches = categories_list.select { |ls| ls['name'] == category_name }
    if matches.size >= 1
      category_id = matches.first['id']
      resp = authorized_http_client.get(categories_url + "/id/#{category_id}",
                                        headers: { 'Accept' => 'application/json' })
      category = JSON.parse(resp.body)['category']
      instance = {
        ensure: :present,
  ...
  def flush_instance
    ...
    when :absent
      ...
      unless cached_instance[:id].nil?
        url = categories_url + "/id/#{cached_instance[:id]}"
    ...
    when :present
      ...
      if cached_instance[:id].nil?
        ...
        authorized_http_client.post(categories_url + '/id/0',
                                    body: body,
                                    headers: { 'Content-Type' => 'application/xml' })
      else
        authorized_http_client.put(categories_url + "/id/#{cached_instance[:id]}",
                                   body: body,
                                   headers: { 'Content-Type' => 'application/xml' })
      end
    end
  ...
  def categories_url
    ...
    @categories_url ||= "#{resource[:api_url]}/JSSResource/categories"
  end
  ...
  #       with a value of true along with an category_name attribute for cloud
  #       server management to operate correctly.
  def category_name
    resource[:is_cloud] ? resource[:category_name] : resource[:name]
  end
end
