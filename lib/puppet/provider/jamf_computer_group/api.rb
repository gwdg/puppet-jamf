...
Puppet::Type.type(:jamf_computer_group).provide(:api, parent: Puppet::Provider::Jamf) do
  ...
  def read_instance
    resp = authorized_http_client.get(computer_group_url,
                                      headers: { 'Accept' => 'application/json' })
    body_json = JSON.parse(resp.body)
    groups_list = body_json['computer_groups']

    # find the category that matches our name
    matches = groups_list.select { |ls| ls['name'] == group_name }
    if matches.size >= 1
      group_id = matches.first['id']
      resp = authorized_http_client.get(computer_group_url + "/id/#{group_id}",
                                        headers: { 'Accept' => 'application/json' })
      group = JSON.parse(resp.body)['computer_group']
      instance = {
        ensure: :present,
  ...
  def flush_instance
    ...
    when :absent
      ...
      unless cached_instance[:id].nil?
        url = computer_group_url + "/id/#{cached_instance[:id]}"
    ...
    when :present
      ...
      if cached_instance[:id].nil?
        ...
        authorized_http_client.post(computer_group_url + '/id/0',
                                    body: body,
                                    headers: { 'Content-Type' => 'application/xml' })
      else
        authorized_http_client.put(computer_group_url + "/id/#{cached_instance[:id]}",
                                   body: body,
                                   headers: { 'Content-Type' => 'application/xml' })
      end
    end
  ...
  def computer_group_url
    # create a URL based on our API URL
    @computer_group_url ||= "#{resource[:api_url]}/JSSResource/computergroups"
  end
